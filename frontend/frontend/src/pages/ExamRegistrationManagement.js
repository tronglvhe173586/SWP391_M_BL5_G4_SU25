...
const ExamRegistrationManagement = () => {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      setError('Lỗi khi tải danh sách đăng ký: ' + (error.response?.data?.message || error.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusUpdate = async (registrationId, newStatus) => {
    try {
      ...
      if (response.data && response.data.code === 1000) {
        setSuccess(`Đăng ký đã được ${newStatus === 'CONFIRMED' ? 'chấp thuận' : 'từ chối'} thành công.`);
        fetchRegistrations();
      } else {
        setError('Không thể cập nhật trạng thái.');
      }
    } catch (error) {
      setError('Lỗi cập nhật trạng thái: ' + (error.response?.data?.message || error.message));
    }
  };
  ...
  const handleDelete = async (registrationId) => {
    try {
      ...
      if (response.data && response.data.code === 1000) {
        setSuccess('Đăng ký đã được xóa thành công.');
        fetchRegistrations();
      } else {
        setError('Không thể xóa đăng ký.');
      }
    } catch (error) {
      setError('Lỗi khi xóa đăng ký: ' + (error.response?.data?.message || error.message));
    }
  };
        ...
        <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            ...
            <Button
                ...
                disabled={loading}
            >
              {loading ? <CircularProgress size={24} /> : 'Làm mới'}
            </Button>
          </Box>
